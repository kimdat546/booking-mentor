...
const ListMentor = () => {
    const { pendingList, getMentorsPending } = useContext(UserContext);
    ...
    const [dataList, setDataList] = useState(pendingList);
    let [page, setPage] = useState(1);

    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            pendingList.length == 0 && (await getMentorsPending());
            setIsLoading(false);
        };
        fetchData();
    }, []);
    ...
    return (
        <Dashboard title={"List Mentor Pending"}>

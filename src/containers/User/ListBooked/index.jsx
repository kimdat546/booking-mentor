...
const ListMentor = () => {
    const { bookedList, getMentorsBooked } = useContext(UserContext);
    ...
    const [dataList, setDataList] = useState(bookedList);
    let [page, setPage] = useState(1);

    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            bookedList.length == 0 && (await getMentorsBooked());
            setIsLoading(false);
        };
        fetchData();
    }, []);
    ...
    const handleDone = async () => {
        handleClose();
        setIsLoading(true);
        await API.postDoneMentor(userSelected.orderId)
            .then(async (res) => {
                setIsLoading(false);
                if (res.status === 200) {
                    Utils.popupAlert({
    ...
    return (
        <Dashboard title={"List Mentor Booked"}>
